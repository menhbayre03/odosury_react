...
class Lesson extends Component {
	...
	render() {
		const {
			main: { user = {}, premiumPrice, eishPrice },
			lesson: { lesson, rating, lessonLoading },
			dispatch
		} = this.props;
		let mediaUrl = "";
		if (lesson.video) {
				...
				<div className="lesson-single">
					<Loader status={lessonLoading}>
						<div
							...
							style={{
								backgroundImage: (lesson.thumbnail || {}).path
									? `url('${(lesson.thumbnail || {}).url}${(lesson.thumbnail || {}).path}')`
									: "none"
							}}
						>
								...
								{isMobile ? (
									lesson.video ? (
										<div className="vid-rom mobi" onClick={() => this.openModalVid()}>
											<img
												src={
													(lesson.thumbnailSmall || {}).path
														? `${(lesson.thumbnailSmall || {}).url}${lesson.thumbnailSmall.path}`
														: "/images/default-lesson.jpg"
												}
												onError={(e) => (e.target.src = `/images/default-lesson.jpg`)}
												style={{
													height: "auto",
													margin: "0 40px",
													width: "calc(100% - 80px)",
													...
													>
														...
														>
															<ion-icon
																...
																name="people-outline"
															/>{" "}
															{(lesson.progress || []).length} Судалсан
														</span>
													</div>
												</div>
												{isMobile ? (
															...
															<div className="inner">
																{lesson.free ? (
																	...
																	>
																		...
																		>
																			{config.formatMoney(lesson.price)}₮
																		</span>
																		<span
																			style={{
																				marginLeft: 15,
																		...
																		>
																			{config.formatMoney(lesson.sale)}₮
																		</span>
																	</div>
																) : lesson.eish ? (
																	<React.Fragment>
																		...
																		>
																			2021 оны шинэ хөтөлбөр дагуу Амжилт кибер сургуулийн
																			мэргэжлийн багш нараар бэлтгэгдсэн ЭЕШ-ийн хичээлүүд{" "}
																...
																{lesson.free ? (
																	!user ? (
																		<Button
																			variant="primary"
																			onClick={() =>
																				config.get("emitter").emit("openLogin", {
																					type: "lesson",
																					lesson: lesson
																				})
																			}
																	...
																	<Button
																		...
																		onClick={() =>
																			user
																				? config.get("emitter").emit("paymentModal", {
																						type: "lesson",
																						lesson: lesson
																				  })
																				: config.get("emitter").emit("openLogin", {
																...
																{lesson.eish || lesson.free ? null : ((user || {}).wish || []).some(
																		(aa) => aa == lesson._id
																  ) ? (
																	<Button
																		onClick={() => this.addWish(lesson._id, false)}
																		variant="secondary"
																	>
																	...
																	<Button
																		onClick={() => this.addWish(lesson._id, true)}
																		variant="secondary"
																	>
																		...
																		>
																			...
																			>
																				...
																				>
																					{config.formatMoney(
																						premiumPrice -
																							(lesson.sale > 0 ? lesson.sale : lesson.price)
																					) + "₮"}
																				</span>{" "}
																				нэмж төлөөд бүх хичээлийг үзэх боломжтой
																			</p>
																			<Link
																				to="/premium"
																				style={{
						...
						<Container className="inner-cont">
							<Row>
														...
														<div className="teacher-side">
															<img
																src={
																	(lesson.teacher || {}).avatar
																		? `https://odosury.com${lesson.teacher.avatar}`
																		: "/images/default-lesson.jpg"
																}
																onError={(e) => (e.target.src = `/images/default-lesson.jpg`)}
															/>
															<div>
																<h5>
																	Багш:{" "}
																	<span>
																		{lesson.teacher.last_name.charAt(0) +
																			". " +
																		...
																		{(item.programs || []).map((program, ind) =>
																			lesson.paid ? (
																				<Link
																					...
																					to={{
																						pathname: `/lesson/view/${lesson.slug}`,
																						state: {
																							levelIndex: index,
																							programIndex: ind
																						}
																				...
																				>
																					<div
																						className={`program ${
																							(program.passed_users || []).indexOf(
																								((user || {})._id || "WW@@#").toString()
																							) > -1
																								? "passed"
																								: ""
																						}`}
																					...
																					>
																						...
																						<p>
																							...
																							<span>{(program.timeline || {}).description}</span>
																						</p>
								...
								<Col lg={4}>
									{isMobile ? null : (
										<div
											className={`fixeee ${this.state.fixed ? "fixeed" : "not-fixeed"}`}
											style={{ position: "sticky", top: 80, marginTop: -100, marginBottom: 140 }}
										>
													...
													<div className="inner">
														{lesson.free ? (
															...
															<React.Fragment>
																...
																>
																	2021 оны шинэ хөтөлбөр дагуу Амжилт кибер сургуулийн мэргэжлийн
																	багш нараар бэлтгэгдсэн ЭЕШ-ийн хичээлүүд <strong>49'000₮</strong>
																</p>
															</React.Fragment>
														) : (
															<div
															...
															<React.Fragment>
																...
																>
																	...
																	>
																		...
																		нэмж төлөөд манай аппликейшнд байгаа бүх хичээлийг үзэх
																		боломжтой
																	</p>

...
class Bundle extends Component {
    ...
    componentDidMount() {
        if (Object.keys((this.props.main || {}).user || {} || {}).length > 0) {
            ...
            this.setProgram = config
                .get("emitter")
                .addListener("setProgram", function (lessonView) {
                    let aa = 0;
                    setTimeout(function () {
                        if (
                            (location.state || {}).levelIndex != null &&
                            (location.state || {}).programIndex != null
                        ) {
                            let prog = (lessonView.levels || [])[location.state.levelIndex]
                                .programs[location.state.programIndex];
                            self.setState(
                                {
                                    program: prog.timeline,
                                    activeIndex: location.state.levelIndex.toString(),
                                },
                                () => dispatch(actions.setProgress(lessonView._id, prog))
                            );
                        } else {
                            (lessonView.levels || []).map((item, index) => {
                                item.programs.map((prog, indP) => {
                                    if (
                                        (prog.passed_users || []).indexOf(user._id) < 0 &&
                                        aa === 0
                                    ) {
                                        aa = 1;
                                        self.setState(
                                        ...
                                        index + 1 === (lessonView.levels || []).length &&
                                        indP + 1 === item.programs.length &&
                                        aa === 0
                                    ) {
                                        aa = 1;
                                        self.setState(
                                            {
                                                program: (lessonView.levels || [])[0].programs[0]
                                                    .timeline,
                                                activeIndex: "0".toString(),
                                            },
                                            () =>
                                                dispatch(actions.setProgress(lessonView._id, prog))
                                        );
    ...
    selectProgram(program) {
        ...
        this.setState({ program: program.timeline }, () =>
            dispatch(actions.setProgress(lessonView._id, program))
        );
    }
                ...
                {isMobile ? (
                    ...
                    <div className="view-area">
                        ...
                        <Loader status={loadingView}>
                            <div
                                className="view-cont"
                                style={{ marginLeft: this.state.collapse ? 380 : 0 }}
                            >
                                {this.renderBody()}
                            </div>
                        </Loader>
    ...
    renderBody() {
        ...
        return (
            <React.Fragment>
                ...
                <div
                    className={`program-cont ${program.type === "video" ? "video" : ""}`}
                    style={{
                        padding: isMobile
                            ? "20px 0 0 0"
                            : program.type === "video" || program.type === "pdf"
                            ? "40px 0 0 0"
                            : "40px 30px",
                    }}
                >
                    {program._id ? (
                        <div>
                            {program.type === "video" ? null : (
                                <h6 className="tit" style={{ marginTop: 20 }}>
                                    {program.title}
                                </h6>
                            )}
                            {program.content ? (
                                <div
                                    className="content-prog"
    ...
    renderSide() {
        ...
        return (lessonView.levels || []).map((item, index) => (
            <Card key={index}>
                <Card.Header>
                    <Accordion.Toggle
                        onClick={() =>
                            ...
                            <React.Fragment>
                                <div
                                    ...
                                    className={`program ${
                                        ...
                                        this.state.program._id == (program.timeline || {})._id
                                            ? "current"
                                            : ""
                                    }`}
                                    key={ind}

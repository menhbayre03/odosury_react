	...
	render() {
		...
		return (
			<div>
						...
						<Row className="header-top">
							<Col md={6} sm={isMobile ? 8 : 9} xs={isMobile ? 8 : 9} className="section-1">
								...
								>
									...
									<ul
										style={{
											visibility: this.state.cate ? "visible" : "hidden",
											opacity: this.state.cate ? 1 : 0
										}}
									>
										{categories.map((item) => (
											<li key={item._id}>
												<Link to={`/lessons/${item.slug}`}>{item.title}</Link>
											</li>
										))}
									</ul>
								</div>
				...
				<div
					onClick={() => this.setState({ cate: false })}
					style={{
						visibility: this.state.cate ? "visible" : "hidden",
						opacity: this.state.cate ? 0.5 : 0
					}}
					className="cate-back"
				/>
			</div>

...
import AmjiltVideo from "../lesson/video";
...
class ViewAreaAudio extends Component {
    ...
    componentDidMount() {
        ...
        this.setProgram = config
            .get("emitter")
            .addListener("setProgramAudio", function (lessonView) {
                let aa = 0;
                setTimeout(function () {
                    if ((location.state || {}).programIndex != null) {
                        let prog = (lessonView.programs || [])[location.state.programIndex];
                        self.setState({ program: prog.timeline, activeIndex: "0".toString() }, () =>
                            dispatch(actions.setProgressAudio(lessonView._id, prog))
                        );
                    } else {
                        (lessonView.programs || []).map((prog, indP) => {
                            if ((prog.passed_users || []).indexOf(user._id) < 0 && aa === 0) {
                                aa = 1;
                                self.setState({ program: prog.timeline, activeIndex: "0" }, () =>
                                    dispatch(actions.setProgressAudio(lessonView._id, prog))
                                );
                                ...
                                self.setState(
                                    {
                                        program: (lessonView.programs || [])[0].timeline,
    ...
    selectProgram(program) {
        ...
        this.setState({ program: program.timeline }, () =>
            dispatch(actions.setProgressAudio(lessonView._id, program))
        );
    }

...
export default (state = initialState, action) => {
	switch (action.type) {
		...
		case removeSingleOrts.REQUEST:
			let lssns = state.bundle.lessons || [];
			if (action.json.name === "lessons") {
				lssns.map(function (run, idx) {
					if (idx === action.json.index) {
						run = run.filter((les) => les !== action.json.lessonId);
					}
				});
			} else if (action.json.name === "bundleLevel") {
				lssns = lssns.filter((lvl, idx) => idx !== action.json.index);
			}
			return {
				...state,
				bundle: {
				...
				bundleLvl.map(function (run, idx) {
					if (idx === action.json.index) {
						if (run.lessons && run.lessons.length > 0) {
							if (
								!run.lessons.some(
									(check) => check === state.lessonValue.value
								)
							) {
		...
		case addBundleLevel.REQUEST:
			...
			if (bundle.levels && bundle.levels.length > 0) {
				bundle.levels.push({
					title: state.bundleLevelName,
					description: state.bundleLevelDescription,
					lessons: []
				});
			} else {
				bundle.levels = [
			...
			if (action.json.success) {
				if (action.json._id) {
					return {
						...
						bundles: state.bundles.map(function (run) {
							if (run._id === action.json._id) {
								run = {
									...action.json.data,
									thumbnail: action.json.data.bundleThumbnail
								};
							}
							return run;
						})
					...
					return {
						...
						openModal: false,
						all: state.all + 1,
						bundles: [action.json.data, ...state.bundles]
					};
				}
			} else {
				return {

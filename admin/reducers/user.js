...
export default (state = initialState, action) => {
	switch (action.type) {
			...
			if (action.json.success) {
				if (action.json._id) {
					...
					return {
						...
						openModal: false,
						all: state.all + 1,
						users: [action.json.data, ...state.users]
					};
				}
			} else {
				return {
		...
		case updateUserStatus.REQUEST:
			return {
				...state,
				updatingStatus: true
			};
		case updateUserStatus.RESPONSE:
			if (action.json.success) {
				let asd = [];
				state.users.map((usr) => {
					if (usr._id === action.json.data._id) {
						usr.status = action.json.data.status;
					}
					asd.push(usr);
				});
				return {

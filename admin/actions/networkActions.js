    ...
    return (dispatch) => {
        ...
        return fetch(currentUrl, {
            ...
            .then(function (response) {
                if (response.status == 200) {
                    ...
                    if (response.status == 404) {
                        if (config.get("emitter")) config.get("emitter").emit("not-found");
                    }
                    if (response.status == 401) {
                        if (config.get("emitter")) config.get("emitter").emit("auth-error");
                    }
                    return {
                        success: false,
            ...
            .catch((error) => {
                dispatch(
                    requestEnd(
                        { success: false, error: error, data: requestParams },
                        requestActions
                    )
                );
            });
...
export function requestPost(requestActions, url, data, requestParams = null, header = {}) {
    return (dispatch) => {
        ...
        return fetch(currentUrl, option)
            ...
            .catch((error) => {
                return dispatch(
                    requestEnd({ success: false, error: error, data: data }, requestActions)
                );
            });
    };
}

export function uploadProgressMediaLib(
    requestActions,
    data,
    type,
    neededData = {},
    ...
    return (dispatch) => {
        ...
        axios
            ...
            .catch((error) => {
                dispatch(
                    requestMediaLibUploadEnd({ success: false }, requestActions, type, neededData)
                );
            });
    };
}
...
export function uploadProgress(
    ...
    return (dispatch) => {
        ...
        axios
            ...
            .catch((error) => {
                dispatch(
                    requestMediaUploadEnd({ success: false }, requestActions, type, neededData)
                );
            });
    };
}
...
export function requestUploadPostDirect(
    requestActions,
    url,
    header,
    requestParams = null,
    ...
    return (dispatch) => {
        dispatch(
            requestUploadDirectStart({ id: id, file: files[0] }, threadID, requestActions, type)
        );

...
class Bundle extends React.Component {
    ...
    componentDidMount() {
        this.props.dispatch(
            actions.getBundle({ pageNum: this.state.pageNum, pageSize: this.state.pageSize })
        );
    }
    openModal(data) {
        const {
    ...
    onChangeHandler(e) {
        this.props.dispatch(
            actions.bundleChangeHandler({ name: e.target.name, value: e.target.value })
        );
    }
    ...
    submitTeacher() {
        const {
            bundle: { bundle, bundleThumbnail },
        } = this.props;
        if (
            !bundleThumbnail ||
            (bundleThumbnail && bundleThumbnail.path && bundleThumbnail.path.trim() === "")
        ) {
    ...
    addBundleLevel() {
        const {
            bundle: { bundleLevelName, bundleLevelDescription },
        } = this.props;
    ...
    removeSingleOrts(index, name, lessonId) {
        this.props.dispatch(
            actions.removeSingleOrts({ name: name, index: index, lessonId: lessonId })
        );
    }
    addSingleOrts(idx) {
        const {
    ...
    changeState(e, value, idx) {
        if (e === "bundleLevelName") {
            this.props.dispatch(
                actions.bundleLevelOnChange({ name: e, value: value.target.value })
            );
        ...
        return (
            ...
            >
                ...
                <Modal
                    ...
                    maskClosable={false}
                >
                    <Form.Item label="Зураг" labelCol={{ span: 5 }} fieldKey="pic">
                        <div>
                            <Button
                                onClick={this.openMediaLib.bind(this, "image")}
                                style={{ marginBottom: 10 }}
                            >
                                <UploadOutlined />{" "}
                                {bundleThumbnail && bundleThumbnail._id ? "Солих" : "Зураг"}
                            </Button>
                            {bundleThumbnail && bundleThumbnail._id ? (
                                <div className="uploaded-i">
                                    <span className="uploaded-i-image">
                                        <img
                                            src={`${config.get("hostMedia")}${
                                                bundleThumbnail.path
                                            }`}
                                        />
                                    </span>
                                    <span
                    ...
                    <Form.Item label="Нэр" labelCol={{ span: 5 }} fieldKey="tit">
                        <Input
                            maxLength={60}
                    ...
                    <Form.Item label="Тайлбар" labelCol={{ span: 5 }} fieldKey="desc">
                        <TextArea
                            size="middle"
                            rows={4}
                    ...
                    <Form.Item label="Үнэ" labelCol={{ span: 5 }} fieldKey="pri">
                        <InputNumber
                            size="middle"
                            style={{ width: "100%" }}
                            name="price"
                            ...
                            value={
                                bundle.price
                                    ? bundle.price.toString().replace(/\$\s?|(,*)/g, "")
                                    : "0".replace(/\$\s?|(,*)/g, "")
                            }
                    ...
                    <Form.Item label="Хямдрал" labelCol={{ span: 5 }}>
                        <InputNumber
                            size="middle"
                            style={{ width: "100%" }}
                            name="sale"
                            ...
                            value={
                                bundle.sale
                                    ? bundle.sale.toString().replace(/\$\s?|(,*)/g, "")
                                    : "0".replace(/\$\s?|(,*)/g, "")
                            }
                            formatter={(value) => `${value}₮`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                    ...
                    <Form.Item label="Түвшин" labelCol={{ span: 5 }} fieldKey="lev">
                                      ...
                                      <div className="levels" key={idx + "levels"}>
                                          <div className="level-title">
                                              ...
                                              <Button
                                                  ...
                                                  danger
                                                  style={{ float: "right" }}
                                                  onClick={this.removeSingleOrts.bind(
                                                      this,
                                                      idx,
                                                      "bundleLevel",
                                                      ""
                                          ...
                                          <div className="timeline-select">
                                              <Select
                                                  value={
                                                      lessonValue.value && lessonValue.index === idx
                                                          ? lessonValue.value
                                                          : ""
                                                  }
                                                  name="lessons"

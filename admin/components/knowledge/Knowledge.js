...
class Knowledge extends React.Component {
    ...
    componentDidMount() {
        ...
        dispatch(actions.getKnowledge({pageNum: this.state.pageNum, pageSize: this.state.pageSize}));
    }
    ...
    submitKnowledge() {
        const {knowledge:{knowledge, knowledgeImage, knowledgeGallery, knowledgeAudio, removedMedias}, main:{company}} = this.props;
    ...
    deleteKnowledge(id, e){
        ...
        this.props.dispatch(actions.deleteKnowledge({_id:id, pageSize: this.state.pageSize, pageNum: this.state.pageNum}));
    }

    customRequest(files, forWhat) {
        const {main:{user, company}} = this.props;
        // files.file.path = files.file.name;
        if(forWhat === 'knowledgeImage'){
            this.props.dispatch(actions.uploadKnowledgeImage(
                [files.file],
                {
                    type: 'image',
                    forWhat: forWhat,
        ...
        if(forWhat === 'knowledgeAudio'){
            this.props.dispatch(actions.uploadKnowledgeAudio(
                [files.file],
                {
                    type: 'audio',
                    forWhat: forWhat,
    ...
    paginationChangeHandler(e) {
        const { dispatch, main:{company} } = this.props;
    ...
    render(){
        let { main: {company}, knowledge:{ status, openModal, knowledge, knowledges, submitKnowledgeLoader, all, openSubModal, submitSubKnowledgeLoader, categories, loadingCover, knowledgeImage, knowledgeAudio, knowledgeGallery, shop_categories, shop_assets, openSubKnowledgeModal, subKnowledgeParent, subKnowledgeParentTitle, subKnowledgeParentSubAssets, subKnowledges, openSubSubModal, subKnowledge, subKnowledgeLoader} } = this.props;
        const uploadButton = (
            <div style={{fontSize: 24}}>
                {!!(knowledgeImage && knowledgeImage.loading) ? <LoadingOutlined /> : <PlusOutlined />}
            </div>
        );
        ...
        if(knowledgeAudio && knowledgeAudio._id){
            ...
            mediaUrl = hostMed+"/api/admin/audio/show/"+knowledgeAudio._id+'?token='+Cookies.get('token');
        }
        const columns = [
            {
                key: 'num',
            ...
            <Card
                ...
                extra={[
                    <Button type={'primary'}
                            ...
                            icon={<PlusOutlined/>}>Танин мэдэхүй нэмэх</Button>
                ]}
            >
                <div style={{marginBottom: 20}}>
                    <Form onFinish={this.searchKnowledge.bind(this)}>
                        <Input addonAfter={<CloseCircleFilled style={{color:'white'}} onClick={() => this.setState({search:''})} />} maxLength={60} size='small' placeholder='Танин мэдэхүй нэр' style={{width: 200, marginRight: 20}} value={this.state.search} name='search' onChange={(e) => this.setState({search: e.target.value})} />
                        ...
                        {/*</Select>*/}
                        <Button loading={status} type="primary" size='small' icon={<SearchOutlined />} htmlType="submit" >Хайх</Button>
                    </Form>
                </div>
                {knowledges && knowledges.length > 0 ?
                    <Table size="small" dataSource={knowledges} columns={columns} onChange={this.tableOnChange.bind(this)} pagination={pagination}/>
                    :
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item
                                    ...
                                    // labelCol={{span: 3}}
                                >
                                    <Input maxLength={60} value={knowledge.title ? knowledge.title : ''} name='title'
                                           onChange={(e) => this.onChangeHandler('title', e.target.value)} allowClear
                                           autoComplete='off'/>
                                </Form.Item>
                            </Col>
                            <Col span={12}>
                                ...
                                >
                                    ...
                                    <TreeSelect
                                        ...
                                        showSearch
                                        style={{width: '100%'}}
                                        value={(knowledge.category && knowledge.category._id) || knowledge.category}
                        ...
                        <Row gutter={16}>
                            <Col span={24}>
                                <Form.Item
                                    ...
                                    // labelCol={{span: 3}}
                                >
                                    <TextArea value={knowledge.description ? knowledge.description : ''} name='description' rows={4}
                                              onChange={(e) => this.onChangeHandler('description', e.target.value)} allowClear
                                              autoComplete='off'/>
                                </Form.Item>
                            </Col>
                        </Row>
                            ...
                            <Col span={24}>
                                ...
                                >
                                    <div className='knowledge-upload-image'>
                                        {knowledgeImage && knowledgeImage.path && knowledgeImage.path !== '' ?
                                            <React.Fragment>
                                                <img
                                                    onError={(e) => e.target.src = `/images/ulger.jpg`}
                                            ...
                                            <Upload
                                                ...
                                                disabled={!!(knowledgeImage && knowledgeImage.loading)}

                                                beforeUpload={this.beforeUpload.bind(this)}
                                                customRequest={(e) => this.customRequest(e, 'knowledgeImage')}
                                            >
                                                {uploadButton1}
                                            </Upload>
                                        }
                                        {knowledgeImage && !knowledgeImage.loading ?
                                            <Upload
                                                ...
                                                customRequest={(e) => this.customRequest(e, 'knowledgeImage')}
                                            >
                                                <Tooltip title='Зураг солих' key={'cov-ed'} placement='right'>
                                        ...
                                        {knowledgeImage && knowledgeImage.percent !== null && typeof(knowledgeImage.percent) != "undefined" ?
